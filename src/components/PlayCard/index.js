...
class PlayCard extends Component {
  ...
  renderBanner = () => (
    <div className="banner">
      <div className="banner-box">
        <h1 className="singer">Ed Sheeran</h1>
  ...
  deleteSong = id => {
    ...
    const updatedList = trackList.filter(each => each.id !== id)
    this.setState({trackList: updatedList})
  }

  render() {
    const {searchInput, trackList} = this.state
    const searchedResults = trackList.filter(eachSong =>
      eachSong.name.toLowerCase().includes(searchInput.toLowerCase()),
    )
    const len = searchedResults.length === 0
    return (
